# Code reusing snippets from https://github.com/patil-suraj/question_generation
# which itself uses transformers from hugging face for question generation


import json
import nltk
from avatar_models.vqa.pipelines import pipeline
...
def preprocess_qa(qa):
    ...
    if q.endswith(", I can see what?") or q.endswith(", we can see what?") or q.endswith(" we can see what?"):
        ...
        q = q.lower()
        q = "What is " + q
    clicked = ["Where is the picture clicked?", "Where is the image clicked?", "Where is the image clicked on?",
               "Where is this picture clicked?", "Where does the picture appear to be clicked?",
               "What is the image clicked in?"]
    if q in clicked:
...
if __name__ == "__main__":
    ...
    with jsonlines.open(LOCALIZED_NARRATIVES_FILE) as reader:
        annotations = [i for i in iter(reader)]

    # annotations = open(LOCALIZED_NARRATIVES_FILE, "r").readlines()
    ...
    # save to disk
    file_to_save = os.path.join(ADE20K_DIR, 'ade20k_qa_cleaned.json')
    with open(file_to_save, 'w') as fp:
        json.dump(cleaned_qa, fp)
    print(f"Saved the dataset to disk: {file_to_save}")
