...
def run_ade20k_vqa_metrics():
    ...
    questions = df["question"].apply(lambda x: " ".join(text_to_word_sequence(x)[1:-1]))
    answers = df["answers"].apply(lambda x: eval(x))
    image_paths_val = df["image_path"].apply(lambda x: os.path.join(ADE20K_DIR, x))
    total = 0
    vqa = get_eval_vqa_model()
